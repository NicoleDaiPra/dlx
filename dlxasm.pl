...
%instTbl = (
	...
	"sll"     => "r,0x00", # MODIFIED, original was 0x04
	"srl"     => "r,0x06",
	"sra"     => "r,0x07",
	"add"     => "r,0x20",
	"addu"    => "r,0x21",
	...
	"j"    => "j,0x02",
	"jal"  => "j,0x03",
	"beqz" => "b,0x10", # MODIFIED, original was 0x04
	"bnez" => "b,0x11", # MODIFIED, original was 0x05
	"bfpt" => "b0,0x06",
	"bfpf" => "b0,0x07",
	"addi" => "i,0x08",
	"addui" => "i,0x09",
	...
	"rfe"  => "n,0x10",
	"trap" => "t,0x11",
	"jr"   => "jr,0x08", # MODIFIED, original was 0x12
	"jalr" => "jr,0x09", # MODIFIED, original was 0x13
	"slli" => "i,0x14",
	"nop"  => "n,0x00", # MODIFIED, original was 0x15
	"srli" => "i,0x16",
	"srai" => "i,0x17",
	"seqi" => "i,0x18",
	"snei" => "i,0x19",
...
sub forminstr {
	...
	if ($itype =~ /^r/) {
		...
		if ($itype eq "b") {
			...
			$src2 = 0;
			$dst = &getimm ($a[2]);
		} elsif ($itype eq "b0") {	# b0 - branches w/o operands
			$src1 = 0;
			$src2 = 0;
			$dst = &getimm ($a[1]);
		} elsif ($itype eq "b1") { # b1 - bgez and bltz
			# the opcode in the table has to go in the bits 20 downto 16
			# since the real opcode is called "regimm" and has a fixed value of 000001
			$src2 = $op;
			$op = 0x01; # regimm
			$src1 = &getreg ($a[1]);
			$dst = &getimm ($a[2]);
		...
		if ($a[0] eq "jalr") {
			...
			my $rd;
			$rs = &getreg ($a[1]);
			$rd = 31; # ABI return register
			$out = 0x00000000 | ($rs << 21) | ($rd << 11) | $op;
		} elsif ($a[0] eq "jr") {
			my $rs;
			$rs = &getreg ($a[1]);
			$out = 0x00000000 | ($rs << 21) | $op;
		}
	} elsif ($itype eq "t") {
		$dst = &getimm ($a[1]);
		$out = ($op << 26) | ($dst & 0x3ffffff);
